...
│   ├── homepage.py                  # Gradio web application (chat + teleop interface)
│   ├── detect_jenga.py              # HSV color segmentation and contour detection
│   ├── colour_coordinates.py        # Camera capture, block localization, coordinate transform
│   ├── text_classifier.py           # model2vec + Logistic Regression intent classifier
│   ├── fsm_controller.py            # Finite state machine for action validation
│   └── roarm_m2/                    # Arm driver and motion primitives
│       ├── roarm_helper.py          # Low-level arm controller (JSON-over-HTTP, motion completion)
│       └── actions/
│           ├── control_action.py    # High-level action controller
│           ├── pickup.py            # Pick motion sequence
│           ├── place.py             # Place motion sequence
│           └── drop.py              # Drop motion sequence
│
├── calibration/                     # Camera-to-robot calibration
│   ├── arm_calibrate.py             # ArUco-based calibration script
│   ├── calibration_matrix.npy       # Saved homography matrix
│   └── aruco_markers/               # Printable ArUco marker assets
│       ├── generate_markers.py      # Marker generation utility
│       └── marker_*.png             # Pre-generated marker images
│
├── tests/                           # All test and debugging scripts
│   ├── camera_calibrate.py          # Camera calibration test
│   ├── color_segment.py             # Color segmentation test
│   ├── detect_aruco.py              # ArUco detection test
│   ├── hsv_slider.py                # Interactive HSV threshold tuning
│   ├── video_display.py             # Raw video feed display
│   ├── yolo.py                      # YOLO-based detection test
│   ├── https_json.py                # HTTP/JSON communication test
│   ├── position.py                  # Arm position readout test
│   └── test_picknplace.py           # End-to-end pick-and-place test
│
...
│   └── project_transfer_prompt.md   # Project context document
│
├── lib/                             # Prebuilt vendor libraries
│   └── pyrealsense_build/           # pyrealsense2 shared objects (aarch64)
│
├── requirements.txt
├── README.md
